...
bool isPersonAllowed (std::string person)
{
	...
	for (int x = 0; x < (sizeof (ALLOW_ACCESS) / sizeof (char*)); x++)
	{
		...
		{
			is_allowed = true;    // (== to = for assigning the true value)
			break;
		}
	}
...
void ssDoorLockEventHandler (DoorLock::DoorLockEvent event, std::string pin_code) // (convert int type to string)
{
	...
	switch (event)
	{
		...
		{
			if (pin_code == SECURE_PIN_CODE) //(convert the string datatype on rightside into int by using std::stoi & same on left side)
			{
				access_granted = true;
				DoorLock::unlockDoor ();
			}
